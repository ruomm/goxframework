...
func xReflect_transToInt64(origVal interface{}, cpOpt string) interface{} {
	...
	if xIsIntegerKind(actualKind) {
		...
		if strings.HasPrefix(viString, "-") {
			viStr64, err := strconv.ParseInt(viString, numBase, 64)
			if err != nil {
				vi = nil
			} else {
				vi = viStr64
			...
			viStr64, err := strconv.ParseUint(viString, numBase, 64)
			if err != nil {
				vi = nil
			} else {
				vi = int64(viStr64)
		...
		if xIsStructType(actualKind) && xIsTimeType(srcFieldVT) {
			...
			viFloat64 := actualValue.Interface().(time.Time)
			vi = xTransTimeToInt64(&viFloat64, optStr)
		} else {
			vi = nil
		}
	} else {
